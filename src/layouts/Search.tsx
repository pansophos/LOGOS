  ...
  return (
    <div className="min-h-[45vh]">
      <input
        ...
        placeholder="Type here to SEARCH the λόγος project…"
        type="text"
        name="search"
        value={inputVal}
      ...
      <div className="row">
        {searchResults?.map(({ item }) => (
          <div key={item.slug} className={"col-12 mb-8 sm:col-6"}>
            {item.data.image && (
              <a href={`${config.site.base_path}/${item.slug}`} className="rounded-lg block hover:text-primary overflow-hidden group">
                <img
                  ...
                  <ul>
                    {item.data.categories.map((category: string, i: number) => (
                      <li className="inline-block">
                        <a
                          href={`${config.site.base_path}/categories/${slugify(category)}`}
            ...
            <h3 className="mb-2">
              <a href={`${config.site.base_path}/${item.slug}`} className="block hover:text-primary transition duration-300">
                {item.data.title}
              </a>
            </h3>
